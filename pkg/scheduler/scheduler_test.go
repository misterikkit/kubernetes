...
func TestSchedulerNoPhantomPodAfterExpire(t *testing.T) {
	...
	scache := cache.New(100*time.Millisecond, stop)
...
func TestSchedulerNoPhantomPodAfterDelete(t *testing.T) {
	...
	defer close(stop)
	queuedPodStore := clientcache.NewFIFO(clientcache.MetaNamespaceKeyFunc)
	scache := cache.New(10*time.Minute, stop)
...
func TestSchedulerErrorWithLongBinding(t *testing.T) {
	...
	for _, test := range []struct {
		...
		queuedPodStore := clientcache.NewFIFO(clientcache.MetaNamespaceKeyFunc)
		scache := cache.New(test.CacheTTL, stop)
...
func setupTestSchedulerWithOnePodOnNode(t *testing.T, queuedPodStore *clientcache.FIFO, scache cache.Cache,
...
func setupTestScheduler(queuedPodStore *clientcache.FIFO, scache cache.Cache, nodeLister schedulertesting.FakeNodeLister, predicateMap map[string]algorithm.FitPredicate, recorder record.EventRecorder) (*Scheduler, chan *v1.Binding, chan error) {
...
func setupTestSchedulerLongBindingWithRetry(queuedPodStore *clientcache.FIFO, scache cache.Cache, nodeLister schedulertesting.FakeNodeLister, predicateMap map[string]algorithm.FitPredicate, stop chan struct{}, bindingTime time.Duration) (*Scheduler, chan *v1.Binding) {
	algo := core.NewGenericScheduler(
		scache,
		nil,
		nil,
